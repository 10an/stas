...
def prompt_model(model, messages):
...
def get_api_response(model, user_prompt):
    # Add user_prompt to messages list.
    chat_messages.append({"role": "user", "content": user_prompt})

    # Prompt model and get output.
    api_output = prompt_model(model, chat_messages)[1]
...
def calculate_token_cost(input_token_price, output_token_price, api_output):
    total_input_tokens += api_output.usage.prompt_tokens
    total_output_tokens += api_output.usage.completion_tokens
    token_cost = (total_input_tokens * (input_token_price/1000)) + (total_output_tokens * (output_token_price/1000))

    return token_cost


# Function to check if the chat_count has exceeded chat_max.
def check_chat_max(chat_max, currency):
    chat_count += 1
